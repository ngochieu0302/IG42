...
function registerGridView(selector) {
    ...
    // lock dai ly
    $(selector + ' .gridView [data-event="lock"]').click(function () {
        var linkFw = '#Page=' + getParameters("Page", 1) + '&RowPerPage=' + getParameters("RowPerPage", 10);
        rowReLock(urlPostAction, $(this).attr("href").substring(1), escapeHTML($(this).attr("title")), linkFw);
        return false;
    });
    //unlock dai ly
    $(selector + ' .gridView [data-event="unlock"]').click(function () {
        var linkFw = '#Page=' + getParameters("Page", 1) + '&RowPerPage=' + getParameters("RowPerPage", 10);
        rowReUnLock(urlPostAction, $(this).attr("href").substring(1), escapeHTML($(this).attr("title")), linkFw);
        return false;
    });
}
...
function rowReLock(urlPost, arrRowId, rowTitle, urlFw) {
    var titleDia;
    if (arrRowId === "")
        createMessage("Thông báo", "Bạn chưa chọn bản ghi nào");
    else {
        titleDia = "Bạn có chắc chắn dừng hoạt động";
        swal({
            title: titleDia,
            text: rowTitle,
            type: "info",
...
function rowReUnLock(urlPost, arrRowId, rowTitle, urlFw) {
    var titleDia;
    if (arrRowId === "")
        createMessage("Thông báo", "Bạn chưa chọn bản ghi nào");
    else {
        titleDia = "Bạn có chắc chắn Cho phép hoạt động";
        swal({
            title: titleDia,
            text: rowTitle,
            type: "info",
