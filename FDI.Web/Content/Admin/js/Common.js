...
$("#btn_add").click(function (e) {
    e.preventDefault();
    var urlRequest = urlForm + (urlForm.indexOf("?") > -1 ?"&do=Add": "?do=Add");
...
function FdiDialog(url, title) {
    ...
    $.post(url, function (data) {
        $("#dialog-form-4 .modal-title").html(title);
...
function ddMMyyyyhhmmss(totals) {
    ...
    var hh = date.getHours();
    var m = date.getMinutes();
    return (d < 10 ? "0" : "") + d + "/" + (mm < 10 ? "0" : "") + mm + "/" + String(y).substring(2, 4) + " " + (hh < 10 ? "0" : "") + hh + ":" + (m < 10 ? " 0" : "") + m;
}
...
function secondsToMs(ts, id, mm, st, td, dns) {
    if (st <= dns) {
        ...
        $("#counterOrder-" + id + "-").html(secondsToHms(ts - td) + "(" + mm + "')");
    }
    else $("#counterOrder-" + id + "-").html(secondsToHms(ts - td) + "(" + mm + "')");
}
...
function bincountrestaurant(c, idc, list, td, dns, timedo, bedid) {
    ...
    if (list.length > 0) {
        ...
        var date = parseInt(list[0].StartDate) - parseInt(td);
        if (count > 1 && parseInt(list[0].StartDate) - parseInt(timedo) < parseInt(dns)) {
            date = parseInt(list[1].StartDate) - parseInt(td);
            count--;
