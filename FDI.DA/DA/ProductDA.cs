                        ...
                        select new CategoryItem
                        {
                            ...
                            ListProductItem = c.Shop_Product_Detail.Where(m => m.IsDelete == false && m.IsShow == true).OrderByDescending(m => m.ID).Select(m => new ShopProductDetailItem
                            {
                                ...
                                ListProductItem = m.Shop_Product.OrderByDescending(p => p.ID).Where(p => p.IsDelete == false).Select(p => new ProductItem
                                {
                                    ...
                                    CodeSku = p.CodeSku,
                                    PriceNew = p.Shop_Product_Detail.Price * (p.Product_Size == null ? 1000 : p.Product_Size.Value) / 1000,
                                })
                            })
                        };
            return query.ToList();
                        ...
                        select new PacketItem
                        {
                            ...
                            LstProduct = from v in c.DN_Product_Packet
                                         select new ProductItem
                                         {
                                             ...
                                             PriceNew = v.Shop_Product.Shop_Product_Detail.Price * (v.Shop_Product.Product_Size == null ? 1000 : v.Shop_Product.Product_Size.Value) / 1000,
                                         }
                        };
            return query.ToList();
        }
            ...
            var query = (from c in FDIDB.DN_Packet
                         ...
                         select new ModelOrderGetItem
                         {
                             ...
                             ListItem = c.DN_Product_Packet.Select(m => new ProductItem
                             {
                                 ...
                                 PriceNew = m.Shop_Product.Shop_Product_Detail.Price * (m.Shop_Product.Product_Size == null ? 1000 : m.Shop_Product.Product_Size.Value) / 1000,
                             })
                         }).FirstOrDefault();
            return query;
        }
            ...
            var query = (from c in FDIDB.Shop_Product_Detail
                         select new ModelOrderGetItem
                         {
                             ...
                             ListItem = c.Shop_Product.Where(m => m.IsDelete != true).Select(m => new ProductItem
                             {
                                 ...
                                 //Value = m.Value,
                                 PriceNew = m.Shop_Product_Detail.Price * (m.Product_Size == null ? 1000 : m.Product_Size.Value) / 1000,
                             })
                         }).FirstOrDefault();
            return query;
        }
