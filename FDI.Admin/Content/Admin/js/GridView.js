            ...
            if (isConfirm) {
                $.post(encodeURI(urlPost), { "do": "hide", "itemId": "" + arrRowId + "" }, function (data) {
                    if (data.Erros) {
                        ...
                        toastr["success"](data.Message);
                        window.location.href = urlFw + "&type=hide&id=" + arrRowId;
                    }
                });
            }
            swal.close();
           ...
           if (isConfirm) {
               $.post(encodeURI(urlPost), { "do": "Active", "itemId": "" + arrRowId + "" }, function (data) {
                   if (data.Erros) {
                       ...
                       toastr["success"](data.Message);
                       window.location.href = urlFw + "&idActive=" + arrRowId;
                   }
               });
           }
           swal.close();
            ...
            if (isConfirm) {
                $.post(encodeURI(urlPost), { "do": "show", "itemId": "" + arrRowId + "" }, function (data) {
                    if (data.Erros) {
                        ...
                        swal.close();
                        toastr["success"](data.Message);
                        window.location.href = urlFw + "&type=show&id=" + arrRowId;
                    }
                });
            } else {
                swal.close();
          ...
          if (isConfirm) {
              $.post(encodeURI(urlPost), { "do": "delete", "itemId": "" + arrRowId + "" }, function (data) {
                  if (data.Erros) {
                      ...
                      toastr["success"](data.Message);
                      if (urlFw.length > 0) {
                          window.location.href = urlFw + "&do=delete&idDelete=" + arrRowId;
                      }
                  }
              });
          } else {
...
function createCloseMessage(title, message, urlFw) {
    var linkFW = '#Page=' + getParameters("Page", 1) + '&RowPerPage=' + getParameters("RowPerPage", 10) + '&temp=' + Math.random().toString(36).substring(7);
